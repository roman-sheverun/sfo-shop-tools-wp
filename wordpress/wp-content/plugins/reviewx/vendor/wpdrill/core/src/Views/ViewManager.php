...
class ViewManager
{
    ...
    protected bool $enableTemplating = \true;
    ...
    {
        $this->plugin = $plugin;
        $this->enableTemplating = Config::get('view.enable_templating', \true);
    ...
    protected function init()
    {
        ...
        $twigConfig = ['cache' => $this->plugin->getPath(Config::get('view.cache_path', 'storage/cache/views')), 'cache_lifetime' => Config::get('view.cache_lifetime', 0)];
        if (!$this->plugin->isProduction()) {
            $twigConfig['debug'] = \true;
            $twigConfig['auto_reload'] = \true;
        }
        $this->twig = new Environment($this->loader, $twigConfig);
        $this->twig->addExtension(new TwigFunctions());
        $lexer = new Lexer($this->twig, ['tag_comment' => Config::get('view.lexer.tag_comment', ['{#', '#}']), 'tag_block' => Config::get('view.lexer.tag_block', ['{%', '%}']), 'tag_variable' => Config::get('view.lexer.tag_variable', ['{{', '}}']), 'interpolation' => Config::get('view.lexer.interpolation', ['#{', '}'])]);
    ...
    public function compile()
    {
        ...
        $regex = new RegexIterator($iterator, '/\\' . $this->templateExtension . '$/');
        // Match .twig files
