...
class Plugin
{
    ...
    public function make(?callable $fn = null) : void
    {
        ...
        $this->registerPluginHooks($initHandlers['activated'] ?? null, $initHandlers['deactivated'] ?? null, $initHandlers['uninstalled'] ?? null);
    }
    public function bind(string $name, callable $resolver) : void
    {
        $this->builder->addDefinitions([$name => $resolver]);
    }
